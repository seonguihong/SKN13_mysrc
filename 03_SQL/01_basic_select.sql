...
# 범주형 컬럼에서 범주값이 어떻게 구성되있는지 확인 -> select distinct 칼럼 from 테이블
#  select distinct 칼럼, 칼럼 from 테이블 -> 칼럼, 칼럼이 겹치지않는 데이터만 출력
...
dept_name "소속 부서"
from emp;
# 칼럼 as 별칭 으로 표기되는 칼럼명을 변경 가능하다. * as는 생략가능, 띄어쓰기 사용하고싶을때 " " 로 묶어주면 사용가능
...
salary,
comm_pct,
salary * comm_pct #comm_pct 값이 null인 경우 곱해도 null이 나옴.
from emp;

-- 직원 이름(emp_name)과 salary를 조회. salary앞에 '$'를 붙여서 출력.
select emp_name,
concat('$', salary) # 어디든 몇개든 문자열 덧셈 해주는 함수 ex) concat('$',salary , '달러') -> $'salary값'달러 출력
from emp;
...
/* 
-> like '' 안에 쓸수있는 연산자 % -> 0글자 이상의 모든 글자들. 
ex) 'S%' S로 시작하는 모든것. , '%Sha%' Sha가 들어간 모든 것. , '%Sha' Sha로 끝나는 모든 것.
'_A__B_C' 두번째 글자는 A 다섯번째 글자는 B 일곱번째 글자는 C 인 _ 자리에 글자수만 맞춰준 모든 것. 
*/
...
# 특수문자를 입력하고 싶을때 % 사이에 escape 문자를 지정하고 사용해서 escape 문자 다음에 오는 특수문자를 일반문자로 만들어준다.
...
select  emp_id, emp_name,hire_date  from emp where hire_date like '2004%';
...
select * from emp where job = 'SA_MAN' and salary <= 13000;
...
from emp 
where job like '%man%' and dept_name = 'Shipping' and hire_date > '2004-12-31';
...
# order by n; select한 컬럼 순번으로 n번째의 컬럼값을 지정함 
#순서가 정해져있기 때문에 마음대로 순서를 바꿔서 쓰면 오류남.
