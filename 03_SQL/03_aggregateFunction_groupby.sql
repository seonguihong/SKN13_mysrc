...
- 문자타입/일시타입: max(), min(), count()에만 사용가능숫자<대문자<소문자<한글 ex) A < 가 -> min = A, max = 가
	- 문자열 컬럼의 max(): 사전식 배열에서 가장 마지막 문자열, min()은 첫번째 문자열.
    - 숫자끼리의 문자열 비교는 첫번째 숫자부터 크기 비교를해서 비교함 '10000'  < '600'
	- 일시타입 컬럼은 오래된 값일 수록 작은 값이다. ex 2025-04-03 < 2025-04-04

******************************************************************************************* */

-- EMP 테이블에서 급여(salary)의 총합계, 평균, 최소값, 최대값, 표준편차, 분산, 총직원수를 조회 
select sum(salary), avg(salary),min(salary), max(salary), stddev(salary), variance(salary), count(*) from emp;
...
select count(distinct ifnull(dept_name, 'a')) from emp; # null값 포함한 결과
...
select emp_name, max(char_length(emp_name)) from emp;
# -> 실행불가능 why? emp_name 107 개를 리턴하고 2번 칼럼은 1개의 데이터가 리턴되는데 표의 형태가 붕괴되서.
# 집계한 결과에 일반적인 컬럼 데이터를 확인 할 수 없음.
...
select job, sum(salary), avg(salary), min(salary), max(salary), stddev(salary), variance(salary), count(*) from emp group by job;

-- 입사연도 별 직원들의 급여 평균.
select year(hire_date), avg(salary) from emp group by year(hire_date) order by 1 asc;
...
select dept_name, job, max(salary), avg(salary) from emp group by dept_name, job;

-- 급여(salary) 범위별 직원수를 출력. 급여 범위는 10000 미만,  10000이상 두 범주.
select if(salary >=10000, '만 이상', '만 미만') , count(*) from emp group by if(salary >=10000, '만 이상', '만 미만');

-- 부서명(dept_name), 업무(job)별 직원수, 최고급여(salary)를 조회. 부서이름으로 오름차순 정렬.
select dept_name, job, count(*), max(salary) from emp group by dept_name, job order by 1 ;

-- 입사년도와(hire_date) 업무(job)가 같은 직원들의 평균 급여(salary)을 조회
select year(hire_date), job, avg(salary) from emp group by year(hire_date), job order by 1;
...
select dept_name, count(dept_name), avg(salary) from emp group by dept_name having count(dept_name)>=10;
...
select dept_name, avg(salary), sum(salary) from emp group by dept_name having avg(salary) >= 5000 and sum(salary) >= 50000;
...
# 위 쿼리문에서 프로그램이 진행 되는 순서
#-> from절 - where절 - group by절 - having 절 - select절 - order by절
