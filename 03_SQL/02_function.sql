...
다중행 함수: 
    ...
    -> 컬럼내의 값을 평균을 내거나 모두 더하거나 하는 모든값을 이용하는 함수
...
select emp_name, lpad(emp_name, 15, ' '), rpad(emp_name, 15, ' ') from emp;
    
--  EMP 테이블에서 이름(emp_name)이 10글자 이상인 직원들의 이름(emp_name)과 이름의 글자수 조회
select emp_name from emp where char_length(emp_name) >=10 ;

select concat('$', format(salary, 3)) salary from emp;
/* **************************************************************************
...
select emp_id, emp_name, salary, ceil(salary *1.15) as SAL_RAISE, ceil(salary *1.15-salary) from emp;
...
select  emp_id, emp_name, comm_pct, comm_pct, round(comm_pct * 1.08, 2) as comm_raise from emp where comm_pct is not null;
...
- 날짜관련 함수
 ...
 dayofweek(날짜): 날짜의 요일을 정수로 반환 (1: 일요일 ~ 7: 토요일) -> 나라마다 달라서 숫자로 리턴
...
-- 날짜 타입에서 년 월 일 조회
select curdate(), year(curdate()), month(curdate()), day(curdate()), dayofweek(curdate());
-- 시간 타입에서 시 분 초 조회
select curtime(), hour(curtime()), minute(curtime()), second(curtime());
-- 특정 기간 만큼 전,후의 일시를 조회
select curdate(), adddate(curdate(), interval 3 year), adddate(curdate(), interval -40 day),subdate(curdate(), interval -3 year);
...
select emp_name, hire_date, date_format(hire_date, '%Y년 %m월 %d일') from emp where emp_id = 200;
--  각 직원의 이름(emp_name), 근무 개월수 (입사일에서 현재까지의 달 수)를 계산하여 조회. 근무개월수 내림차순으로 정렬.
select emp_name, timestampdiff(month, hire_date, now()) from emp order by 2 desc;
...
select emp_id, emp_name, job, ifnull(dept_name, '부서미배치') from emp order by 4 desc;
select emp_id, emp_name, job, if(dept_name is null, '부서미배치', dept_name) from emp;

-- EMP 테이블에서 직원의 ID(emp_id), 이름(emp_name), 급여(salary), 커미션 (salary * comm_pct)을 조회. 커미션이 없는 직원은 0이 조회되록 한다.
select emp_id, emp_name, salary, ifnull(salary*comm_pct, '0') from emp order by 4;
select emp_id, emp_name, salary, if(comm_pct is null, '0',salary*comm_pct) from emp;
-- comm_pct가 없으면 "커미션 없음"을 있다면 "커미션 있음" 으로 출력.
select comm_pct, if(comm_pct is not null,"커미션 있음","커미션 없음") from emp;
...
***********************************************/
/*
명시적으로 변환하지 않아도 알아서 변환이 됨.
select '10' + '20';
->(30) 문자열 + 문자열 이지만 알아서 int 값으로 변환하여 계산함.
select datediff(curdate(), '2025-03-20');
-> date type + 문자열 이지만 문자열을 알아서 date type으로 바뀌어서 계산함.
*/
-- 시간을 정수형태로 변환   
...
when 5 then '목요일'
when 6 then '금요일'
when 7 then '토요일'  end as day;

-- EMP테이블에서 급여와 급여의 등급을 조회하는데 급여 등급은 10000이상이면 '1등급', 10000미만이면 '2등급' 으로 나오도록 조회
select salary, case when salary>=10000 then '1등급' else '2등급' end as grade from emp;
...
select emp_id, emp_name, case job when 'AD_PRES' then '대표' when 'FI_ACCOUNT' then '회계' when 'PU_CLERK' then '구매' end as job 
from emp
