...
# Session State를 생성.
# session_state : dictionary 구현체. 시작 ~ 종료할 때 까지 사용자 별로 유지되야 하는 값을 저장하는 곳.

# 0. 대화 내역을 session_state의 "message" : list로 저장.
# 1. session state에 "message" key가 있는지 조회(없으면 생성)
if "messages" not in st.session_state:
    st.session_state["messages"] = [] # 대화내용을 저장할 리스트를 생성.


st.title("Chatbot 위젯 튜토리얼")

# 사용자의 프롬프트(질문)을 받는 위젯
prompt = st.chat_input("User prompt") # 사용자가 입력한 문자열을 반환

# 대화 작업
if prompt is not None:
    # session_stated에 messages에 대화내역을 저장.
    st.session_state["messages"].append({"role":"user", "content":prompt})
    st.session_state["messages"].append({"role":"ai", "content":ai_message})

# 대화내역을 chat_message containcer에 출력
for message_dict in st.session_state["messages"]:
    with st.chat_message(message_dict['role']):
        st.write(message_dict["content"])


# streamlit run streamlit/01_streamlit_chat_exam.py